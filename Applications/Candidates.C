...
bool Candidate::is_coincident(const Candidate * c)
{
  ...
  const float    sep_snr = 0.50;
  const int64_t tol = sep_time * max(c->filter,filter);

  // change temporal coincidence on bens suggestion 6/8/2012
  return ( (abs((int)(c->sample_idx - sample_idx)) <= tol) &&
...
std::ostream& operator<<(std::ostream& os, const Candidate * c) {
  ...
  /*  os << c->snr << " " << c->sample_idx << " " << c->sample_time << " "
     ...
     << c->nbeams << " " << c->beam_mask << " " << c->primary_beam << " "
     << c->max_snr <<" " << c->beam;*/
  os << c->max_snr << " " << c->sample_idx << " " << c->sample_time << " " << c->filter << " " << c->dm_trial << " " <<  c->dm << " " << c->members << " " << c->primary_beam << " " << c->beam << " " << c->nbeams << " " << c->snr;
  return os;
}
...
void CandidateChunk::compute_coincidence()
{
  ...
  unsigned int rfi_mask = 1 << 32;
  unsigned int beam_thresh = 16;
...
void CandidateChunk::write_coincident_candidates()
{
  ...
  std::string * filename;
  filename = new std::string("/mnt/nfs/data/bfheimdall/coincident.cand");
  ...
  int ibeam=-1, jcand=-1;
  float maxSNR = 0.;
  // find maximum snr, or crab
  for (unsigned i=0; i< n_beams; i++) {
    for (unsigned j=0; j<cands[i].size(); j++) {

      if (cands[i][j]->dm < 60.0) {
	if (cands[i][j]->snr + 1e6 > maxSNR) {
	  maxSNR = cands[i][j]->snr + 1e6;
	  ibeam = i;
	  jcand = j;
	}
      }
      else {
	if (cands[i][j]->snr > maxSNR && cands[i][j]->snr > 6.45) {
	  maxSNR = cands[i][j]->snr;
	  ibeam = i;
	  jcand = j;
	}
      }
  ...
  cout << ibeam << " " << jcand << endl;
  if (ibeam >=0 && jcand >=0) {
    sprintf(ss,"echo %lu-%g-%d-%g | nc -4u -w1 10.10.1.7 11223 &",cands[ibeam][jcand]->sample_idx,cands[ibeam][jcand]->snr,cands[ibeam][jcand]->filter,cands[ibeam][jcand]->dm);
    cout << "Sending to dsa1: " << ss << endl;
  }
  cout << "here" << endl;
